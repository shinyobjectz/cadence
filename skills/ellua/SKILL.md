---
name: ellua
description: Author, edit, preview, and render programmatic videos and motion graphics with ellua — the Lua + LÖVE2D (love2d) browser-free alternative to Remotion/HyperFrames, where a composition is a pure-Lua file and `ellua render comp.lua -o out.mp4` produces the video. Load this skill whenever asked to create, edit, animate, debug, or render a video, motion graphic, title card, caption/text overlay, vertical short, or any animation using ellua, LÖVE/love2d, or Lua compositions, or when touching .lua comp files in this repo. Covers the comp API (rect/circle/text/video/image/svg/lottie nodes, script/tween/ease timeline), the render/preview/hash CLI, determinism rules, and the mandatory render-verification loop.
---

# ellua — agent authoring guide

ellua renders video from Lua. No browser, no React, no project scaffolding: one
