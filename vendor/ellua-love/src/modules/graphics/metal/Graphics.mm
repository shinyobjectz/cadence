...
{ @autoreleasepool {
	...
	if (window != nullptr)
	{
		...
		// Recreate the window using the current renderer, if needed. Headless
		// mode intentionally has no SDL window, but still needs setWindow to
		// establish its virtual backbuffer after the graphics module exists.
		if (window->isOpen() || std::getenv("ELLUA_HEADLESS") != nullptr)
		{
			int w, h;
...
void Graphics::backbufferChanged(const BackbufferSettings &settings)
{
	...
	bool presentationchanged = settings.presentation != backbufferSettings.presentation;
	...
	if (sizechanged || presentationchanged)
	{
		virtualBackbuffer = nil;
		if (settings.presentation == BackbufferSettings::PRESENTATION_HEADLESS)
		{
			auto desc = [MTLTextureDescriptor new];
			desc.width = settings.pixelWidth;
			desc.height = settings.pixelHeight;
			desc.pixelFormat = isGammaCorrect() ? MTLPixelFormatBGRA8Unorm_sRGB : MTLPixelFormatBGRA8Unorm;
			desc.textureType = MTLTextureType2D;
			desc.mipmapLevelCount = 1;
			desc.sampleCount = 1;
			desc.storageMode = MTLStorageModePrivate;
			desc.usage = MTLTextureUsageRenderTarget | MTLTextureUsageShaderRead;
			virtualBackbuffer = [device newTextureWithDescriptor:desc];
			if (virtualBackbuffer == nil)
				throw love::Exception("Out of graphics memory creating headless backbuffer.");
			virtualBackbuffer.label = @"Ellua headless backbuffer";
		}
	}
...
id<MTLRenderCommandEncoder> Graphics::useRenderEncoder()
{
	if (renderEncoder == nil)
	{
		...
		{
			id<MTLTexture> backbufferTexture = nil;
			if (backbufferSettings.presentation == BackbufferSettings::PRESENTATION_HEADLESS)
			{
				backbufferTexture = virtualBackbuffer;
			}
			else if (activeDrawable == nil)
			{
			...
			if (backbufferTexture == nil && activeDrawable != nil)
				backbufferTexture = activeDrawable.texture;
			...
			if (backbufferMSAA.get())
			{
				...
				passDesc.colorAttachments[0].resolveTexture = backbufferTexture;
			}
			else
			{
				attachmentStoreActions.color[0] = MTLStoreActionStore;
				passDesc.colorAttachments[0].texture = backbufferTexture;
...
{ @autoreleasepool {
	...
	id<MTLTexture> maintexture = nil;
	if (backbufferSettings.presentation == BackbufferSettings::PRESENTATION_HEADLESS)
		maintexture = virtualBackbuffer;
	else if (activeDrawable != nil)
		maintexture = activeDrawable.texture;
	...
	if (!pendingScreenshotCallbacks.empty())
	{
		...
		[blitencoder copyFromTexture:maintexture
						 sourceSlice:0
						 sourceLevel:0
	...
	if (backbufferSettings.presentation != BackbufferSettings::PRESENTATION_HEADLESS && cmd != nil && activeDrawable != nil)
