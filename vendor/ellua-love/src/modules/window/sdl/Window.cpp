...
bool Window::setWindow(int width, int height, WindowSettings *settings)
{
	...
	// Ellua's offline renderer needs a GPU context and backbuffer but must not
	// present a user-visible application window. This stays opt-in so normal
	// LÖVE games and the Ellua preview command keep their existing behaviour.
	const char *headlessenv = std::getenv("ELLUA_HEADLESS");
	const bool elluaheadless = headlessenv != nullptr && headlessenv[0] != '0';
	...
	if (elluaheadless)
	{
		// A real headless renderer has no SDL_Window, display, drawable, or
		// swapchain. Keep window dimensions as logical backbuffer settings so
		// the public graphics API remains usable, but do not fabricate a native
		// window handle for love.window calls.
		if (isOpen())
			close();
		...
		if (graphics.get())
		{
			...
			backbufferSettings.presentation = graphics::Graphics::BackbufferSettings::PRESENTATION_HEADLESS;
			...
			{
				throw love::Exception("Failed to initialize headless graphics.\n%s", e.what());
			}
		}
